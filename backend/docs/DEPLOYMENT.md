...
Run Redis alongside PostgreSQL locally with `docker compose up -d`. Start the API with `npm run dev` and the worker separately with `npm run worker:dev`. In production deploy a second process from the same backend image using `npm run worker:start`.

`REDIS_URL` is required in production. The worker uses BullMQ with five attempts and exponential backoff, retains failed jobs for inspection, and records a durable status in PostgreSQL. Admins can inspect queue health, metrics, and retry failed jobs under `/api/operations`.

Attachments are private and remain unavailable until their scan job marks them clean. Until an approved malware-scanner adapter is configured, the worker quarantines completed uploads rather than trusting a client-provided clean status.